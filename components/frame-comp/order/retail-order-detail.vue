      ...
      <el-form class="form form-show" label-position="left" label-width="120px" ref="form">
        ...
        <div class="table-container">
          <el-table size="small" :data="(orderData.order_goods || [])" border style="width: 100%">
            ...
            <el-table-column prop="cost_price" label="批发价" :formatter="util.tableRender()">
            </el-table-column>
        ...
        <div class="order-price">
          <div> <label>批发总价</label><span class="order-price-text">{{util.dataRender(orderData.cost_price_amount,'fixed')}}元</span></div>
  ...
  computed: {
    finallyPrice() {
      if (this.orderData.goods_price_amount) {
        return util.dataRender(parseFloat(this.orderData.cost_price_amount) + parseFloat(this.orderData.shipping_fee) - parseFloat(this.orderData.middle_fee), 'fixed')
      }
    }
  },
};

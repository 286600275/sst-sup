...
<template>
  <div>
    ...
    <div class="table-container">
      <el-table size="small" :data="orderListData" style="width: 100%" id="orderTable" tooltip-effect="light">
          ...
          <template slot-scope="scope">
            <div class="table-container">
              <el-table size="small" :data="(scope.row.order_goods || [])" border style="width: 100%">
                ...
                <el-table-column prop="goods_price" label="零售价" :formatter="util.tableRender()">
                </el-table-column>
        ...
        <el-table-column prop="goods_price_amount" label="零售总价" :formatter="util.tableRender()">
        </el-table-column>
        <el-table-column prop="cost_price_amount" label="批发总价" :formatter="util.tableRender()">
        </el-table-column>
      ...
      <Pagination ref="retailOrderPagi" @render="renderTableData"></Pagination>
    </div>
    <RetailOrderDetail ref="retailOrderDetail"></RetailOrderDetail>
...
import RetailOrderDetail from '~/components/frame-comp/order/retail-order-detail'
...
export default {
  props: [],
  components: { InputWrapper, Pagination, RetailOrderDetail, InvoiceImages, SendGoodsForm },
  data() {
    return {
      util: util,
  ...
  methods: {
    ...
    actionShowInvoice(row) {
      this.$refs['invoiceImages'].show(row, "retail");
    },
    actionShowOrderDetail(row) {
      this.$refs['retailOrderDetail'].show(row, "retail");
    },
    actionRefund(row) {},
    actionCancleOrder(row) {
      ...
      this.$confirm('确定取消订单?', '提示', {
        ...
        util.ajax({
          type: "GET",
          url: util.ajaxPath() + "/order/cancel/" + row.id,
    ...
    actionSendGoods(row) {
      this.$refs['sendGoodsForm'].show(row, 'retail')
    },
    renderTableData() {
      let self = this;
      let param = {
        page: self.$refs['retailOrderPagi'].page,
        per_page: self.$refs['retailOrderPagi'].pageSize,
      };
      ...
      util.ajax({
        ...
        success: function(res) {
          if (res.status == "success") {
            ...
            util.setTotal(self, 'retailOrderPagi', res.total)
          }
        }
      })
    },
    downloadExcel() {
      ...
      var titleArr = [
        //
        "ID", "订单编号", "下单时间", "交易时间", "成交总价", "零售总价", "批发总价", "订单总价", "服务费", "商品数量", "订单状态", "购买人", "电话", "用户ID", "收货地址"
      ];
      var dataArr = [];
      for (var i = 0, item = null; item = self.orderListData[i++];) {
        ...
        rowData.push(util.dataRender(item.goods_price_amount)); //零售总价
        rowData.push(util.dataRender(item.cost_price_amount)); //批发总价
        ...
        rowData.push(util.dataRender(item.name)); //购买人
        rowData.push(util.dataRender(item.mobile)); //电话
        rowData.push(util.dataRender(item.member_id)); //用户ID
        rowData.push(util.dataRender(item.address)); //收货地址
        dataArr.push(rowData);
      }
      util.downloadExl(titleArr, dataArr, undefined, null, "兽兽淘-零售订单列表导出.xlsx")
    },
  },
  mounted() {
    // return;
